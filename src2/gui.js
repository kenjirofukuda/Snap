...
MorphicAnimation, BoxMorph, BlockDialogMorph, RingMorph, Project, ZERO, BLACK*/
    ...
    function autoRun () {
        ...
        if (isLoadingAssets()) {
            myself.world().animations.push(
                new MorphicAnimation(nop, nop, 0, 200, nop, autoRun)
            );
        } else {
            myself.runScripts();
        }
...
IDE_Morph.prototype.scrollPaletteToCategory = function (category) {
    ...
    if (delta === 0) {return; }
    this.world().animations.push(new MorphicAnimation(
        y => { // setter
